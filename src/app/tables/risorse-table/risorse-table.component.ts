...
@Component({
  ...
  templateUrl: './risorse-table.component.html',
  styleUrl:    './risorse-table.component.css'
})
export class RisorseTableComponent implements OnInit
{
   ...
   constructor (public router: Router,
                ...
                public theDataService: RisorseService,
   ...
   onAdd (): void
   {
      this.router.navigate ([`/risorsaedit`], {state: {id: 0}});
   }


   onEdit (item: any): void
   {
      this.router.navigate ([`/risorsaedit`], {state: {id: item.id}});
   }
   ...
   {
      const dlgData: MessDlgData = {
         title:               'Cancellazione Società',
         subtitle:            '',
         message:             `Sei veramente sicuro di voloer cancellare la società '<b>${item.nome}</b>' dal database?`,
         messtype:            'warning',
         btncaption:          'Annulla',
   ...
   ColoreRisorsa (risorsa: Resource): any
   {
      return {'background-color': `${risorsa.coloresfondo}`, 'color': `${risorsa.coloretesto}`};
   }
